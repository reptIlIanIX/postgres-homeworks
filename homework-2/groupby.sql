...
SELECT order_id, customer_id, freight, ship_country FROM ORDERS WHERE ship_city LIKE 'P%' ORDER BY freight DESC LIMIT 10

-- 3. фамилию и телефон сотрудников, у которых в данных отсутствует регион (см таблицу employees)
SELECT first_name, last_name, home_phone WHERE ship_region is NULL

-- 4. количество поставщиков (suppliers) в каждой из стран. Результат отсортировать по убыванию количества поставщиков в стране
SELECT country, COUNT(suppliers) as count FROM suppliers GROUP BY country ORDER BY count DESC
